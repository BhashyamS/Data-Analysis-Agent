"""Build compact evidence for the AI Analyst."""
...
def _records(frame: pd.DataFrame, limit: int = 25) -> list[dict[str, Any]]:
    if frame.empty:
        return []
    safe = frame.head(limit).where(pd.notna(frame), None)
...
def build_ai_context(
    ...
    *,
    ...
    """Return computed summaries only. Raw rows are never included."""
